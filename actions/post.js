...
const buildProductPaths = ({ data, result }) => {
  const productId =
    result?.id ?? result?.product?.id ?? result?.data?.id ?? data?.id;
  if (!productId) return [];
...
export async function postData(data, endpoint, tag, revalidate) {
  ...
  try {
    ...
    if (response.ok && !result?.error) {
      ...
      if (tag === "product") {
        paths.push(...buildProductPaths({ data, result }));
      }
